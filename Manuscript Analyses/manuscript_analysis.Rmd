...
# Removing Participant 323 from the dataset (missing demographic and cognitive data)
...
A total of 60 older adult listeners participated in the study. The overall demographic information for the participants are in the table below.
...
# Customize colors for figures
library(LaCroixColoR) #remotes::install_github("johannesbjork/LaCroixColoR")
my_pal <- lacroix_palette("PeachPear", type = "discrete")
...
The plots below are showing the effects of words in noise hearing threshold and working memory on initial intelligibility for AM1.
...
The figures below are plotting the interaction effects for the PDM10 initial intelligibility models.
...
-   The interaction between inhibitory control and words in noise hearing threshold indicate that inhibitory control helped compensate for listeners with slightly elevated words in noise hearing thresholds. However, inhibitory control did not have a great effect on initial intelligibility for listeners with low or elevated hearing thresholds.
-   The interaction between receptive vocabulary and processing speed indicates that those with greater processing speed and greater vocabulary had greater initial intelligibility
...
effect_plot(mods2, vocab_c, pattern_c) # Receptive Vocabulary * Processing Speed
effect_plot(mods2, pattern_c, vocab_c) # Processing Speed * Receptive Vocabulary
...
The relative importance of the model variables are shown in the figure below. The most important variable in the model is the interaction between inhibitory control and words in noise hearing threshold followed by the interaction between processing speed and words in noise threshold, the interaction between cognitive flexibility and words in noise hearing threshold, and the interaction between inhibitory control and receptive vocabulary.
...
The the interaction effects for the intelligibility improvement model for AM1 are plotted below.

-   The interaction between inhibitory control and words in noise threshold indicate that the effect of inhibiotry control on intelligibility improvement was reduced for listeners with higher hearing thresholds
-   The interaction between processing speed and words in noise threshold indicate that greater processing speed reduced the effect on intelligibility improvement for listeners with higher thresholds.***
- The interaction between cognitive flexibility and words in noise threshold indicate that greater cognitive flexibility helped compensate for intelligibility improvement for listeners with high thresholds. However, for listeners with lower thresholds, greater cognitive flexibility had a negative effect on improvement.
-   The interaction between inhibitory control and receptive vocabulary indicates that increased vocabulary knowledge had a positive effect on intelligibility improvement for listeners with low and high inhibitory control. However, vocabulary knowledge had a negative effect for listeners with "mid/average" inhibitory control.
...
effect_plot(mods12, flanker_c, best_win) # Inhibitory Control * Words in Noise Threshold
effect_plot(mods12, best_win, flanker_c) # Words in Noise * Inhibitory Control
effect_plot(mods12, pattern_c, best_win, num = 4) # Processing Speed * Words in Noise Threshold
effect_plot(mods12, best_win, pattern_c, num = 4)
effect_plot(mods12, card_sort_c, best_win, num = 5) # Cognitive Flexibility * Words in Noise Threshold
effect_plot(mods12, flanker_c, vocab_c, num = 5) # Inhibitory Control * Receptive Vocabulary
effect_plot(mods12, vocab_c, flanker_c, num = 5) # Receptive Vocabulary * Inhibitory Control
...
The interaction effects for the intelligibility improvement models for PDM10 are plotted below.

-   The interaction between cognitive flexibility and receptive vocabulary indicates that cognitive flexibility had a positive effect on improvement for listeners with higher vocabulary knowledge, but minimal effect for listeners with lower vocabulary knowledge
-   The interaction effect between inhibitory control and processing speed indicates that processing speed has a negative effect on intelligibility improvement for listeners with mid-range inhibitory control. However, processing speed has minimal effect on improvement when listeners have lower or higher inhibitory control scores.***
...
-   The interaction between inhibitory control and words in noise threshold indicates a positive effect of inhibitory control for listeners with low words in noise thresholds but little to no effect for listeners with elevated hearing thresholds (a potential negative effect)
