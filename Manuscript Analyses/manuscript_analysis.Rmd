...
The relative variable importance for the models are shown in the figure below. The interaction between episodic (picture sequence) memory and words in noise hearing threshold, the main effect of words in noise threshold, the interaction between working memory and receptive vocabulary, and the interaction between working memory and processing speed were the most important variables for the AM1 initial intelligibility models.
...
Plotting the interaction effects for the AM1 initial intelligibility models.
...
effect_plot <- function(mod, var, var2 = NULL, int = TRUE, num = 3) {
  ...
  if (isFALSE(int)){
    ...
    dat %>%
      ...
      aes(x = {{var}},
          y= .outcome,
          group = cut_interval({{var2}}, num),
          color = cut_interval({{var2}}, num)) +
      geom_point() +
      geom_smooth(method = "lm") +
...
effect_plot(mods1, pic_seq_c, best_win) # Picture Sequence Memory * Words in Noise
effect_plot(mods1, best_win, pic_seq_c) # Words in Noise * Picture Sequence Memory
effect_plot(mods1, best_win, int = F) # Words in Noise
effect_plot(mods1, list_sort_c, vocab_c) # Working Memory * Receptive Vocab
effect_plot(mods1, vocab_c, list_sort_c) # Receptive Vocab * Working Memory
effect_plot(mods1, list_sort_c, pattern_c) # Working Memory * Processing Speed
effect_plot(mods1, pattern_c, list_sort_c) # Processing Speed * Working Memory
...
Like for AM1, two elastic net models were used to model the predictive relationship between the listener cognitive measures and pretest intelligibility for PDM10: a main effects model and an interaction model. The difference in R-square and RMSE is not significantly different between the two models. However, the R-squared value is higher for the main effects model, indicating a better fit to the data.

The important variables for the PDM10 models is shown below. The interaction between receptive vocabulary and processing speed, the main efffect of words in noise hearing threshold, the interaction between inhibitory control and words in noise hearing threshold, and the interaction between working memory and processing speed were the most important variables in the models (in that order).
...
effect_plot(mods2, vocab_c, pattern_c) # Receptive Vocab * Processing Speed
effect_plot(mods2, pattern_c, vocab_c) # Processing Speed * Receptive Vocab
effect_plot(mods2, best_win, int = F) # Words in Noise
effect_plot(mods2, flanker_c, best_win) # Inhibitory Control * Words in Noise
effect_plot(mods2, best_win, flanker_c) # Words in Noise * Inhibitory Control
effect_plot(mods2, list_sort_c, pattern_c) # Working Memory * Processing Speed
effect_plot(mods2, pattern_c, list_sort_c) # Processing Speech * Working Memory
effect_plot(mods2, vocab_c, pic_seq_c) # Receptive Vocab * Picture Sequence Memory
effect_plot(mods2, pic_seq_c, vocab_c) # Picture Sequence Memory * Receptive Vocab
effect_plot(mods2, card_sort_c, vocab_c) # Cognitive Flexibility * Receptive Vocab
effect_plot(mods2, vocab_c, card_sort_c) # Receptive Vocab * Cognitive Flexibility
...
Two elastic net models were used to model the relationship between the listener cognitive measures and intelligibility improvement (controlling for pretest intelligibility) for AM1: the main effects only model and the model with interactions. Based on the R-squared values, the main effects model is better than the interaction model (this difference approached significance). 

The relative importance of the model variables are shown in the figure below.

```{r, warning = F, message = F}

mod_fun2 <- function(speaker, title) {
  subsetted = cor_df %>%
    dplyr::select(-c(fluid_cog_c, improvement)) %>%
    dplyr::mutate(pwc_diff = resid(lm(posttest ~ pretest))) %>%
    dplyr::select(pwc_diff, ends_with("_c"), speaker) %>%
    tidyr::drop_na(pwc_diff, ends_with("_c"), speaker) %>%
...
Plotting the interaction effects for intelligibility improvement models for AM1.

```{r}

## AM1 Effect Plots

effect_plot(mods12, list_sort_c, vocab_c) # Working Memory * Receptive Vocab
effect_plot(mods12, vocab_c, list_sort_c) # Receptive Vocab * Working Memory
effect_plot(mods12, list_sort_c, int = F) # Working Memory
effect_plot(mods12, pattern_c, int = F) # Processing Speed
effect_plot(mods12, list_sort_c, flanker_c) # Working Memory * Inhibitory Control
effect_plot(mods12, flanker_c, list_sort_c) # Inhibitory Control * Working Memory
...
The relative importance of the model variables is shown in the figure below.
...
Plotting the interaction effects for the intelligibility improvement models for PDM10.

```{r, warning = F, message = F}

## PDM10 Effect Plots

effect_plot(mods22, vocab_c, pic_seq_c) # Receptive Vocab * Picture Sequence Memory
effect_plot(mods22, pic_seq_c, vocab_c) # Picture Sequence Memory * Receptive Vocab
effect_plot(mods22, vocab_c, int = F) # Receptive Vocab
effect_plot(mods22, list_sort_c, flanker_c) # Working Memory * Inhibitory Control
effect_plot(mods22, flanker_c, list_sort_c) # Inhibitory Control * Working Memory
